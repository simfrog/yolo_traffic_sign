...
*/
    ...
    int publish_sign(cv_bridge::CvImagePtr& cv_ptr, Rect roi, const autoware_msgs::DetectedObjectArray::ConstPtr &in_objects)
    {
        ...
        // if(roi.y> matOriginImage.rows || roi.x > matOriginImage.cols) return 0;
        ...
        int nABMaxIdx, nOTHMaxIdx;
        /////Modified
        double dABMaxVal = -DBL_MAX, dOTHMaxVal = -DBL_MAX;
        vector<double> vAccABVal(2), vAccOTHVal(3);
        Mat matResponseMap;
        Mat matABImage = matBinaryImage(rAB & bounds);
        Mat matOTHImage = matBinaryImage(rOTH & bounds);
        // imshow("matABImage",matABImage);
        // imshow("matOTHImage",matOTHImage);
        // waitKey(0);
        for (int j = 0; j < vMatAB.size(); j++) {
            for (int k = -2; k < 3; k++) {
                Mat matCurABImage; 
                // if(IMG_SIZE * (1.0 + 0.2 * k) > matOriginImage.rows || IMG_SIZE * (1.0 + 0.2 * k) > matOriginImage.cols) continue;
